...
class SelectorEventLoopUnixSocketTests(test_utils.TestCase):
    ...
    def test_create_unix_datagram_server_existing_path_sock(self):
        with test_utils.unix_socket_path() as path:
            sock = socket.socket(socket.AF_UNIX, socket.SOCK_DGRAM)
            sock.bind(path)
            sock.close()

            coro = self.loop.create_unix_datagram_server(mock.MagicMock, path)
            transport, protocol = self.loop.run_until_complete(coro)
            transport.close()
            self.loop.run_until_complete(transport.wait_closed())
            assert protocol.connection_made.call_count == 1

    @unittest.skipUnless(hasattr(os, 'fspath'), 'no os.fspath')
    def test_create_unix_datagram_server_pathlib(self):
        with test_utils.unix_socket_path() as path:
            path = pathlib.Path(path)
            coro = self.loop.create_unix_datagram_server(mock.MagicMock, path)
            transport, protocol = self.loop.run_until_complete(coro)
    ...
    def test_create_unix_datagram_server_existing_path_nonsock(self):
        with tempfile.NamedTemporaryFile() as file:
    ...
    def test_create_unix_datagram_server_path_inetsock(self):
        sock = socket.socket()
        with sock:
            coro = self.loop.create_unix_datagram_server(mock.MagicMock,
                                                         path=None, sock=sock)
            with self.assertRaisesRegex(ValueError,
                                        'A UNIX Domain Datagram.*was '
                                        'expected'):
                self.loop.run_until_complete(coro)

    def test_create_unix_datagram_server_path_stream(self):
    ...
    @unittest.skipUnless(hasattr(socket, 'SOCK_NONBLOCK'),
                         'no socket.SOCK_NONBLOCK (linux only)')
    def test_create_unix_datagram_server_path_stream_bittype(self):
        sock = socket.socket(
    ...
    def test_create_unix_datagram_connection_path_inetsock(self):
        sock = socket.socket()
        with sock:
            coro = self.loop.create_unix_datagram_connection(mock.MagicMock,
                                                             path=None,
                                                             sock=sock)
            with self.assertRaisesRegex(ValueError,
                                        'A UNIX Domain Datagram.*was '
    ...
    @mock.patch('asyncio.unix_events.socket')
    def test_create_unix_datagram_server_bind_error(self, m_socket):
        # Ensure that the socket is closed on any bind error
        sock = mock.Mock()
        m_socket.socket.return_value = sock
    ...
    def test_create_unix_datagram_connection_path_sock(self):
        coro = self.loop.create_unix_datagram_connection(
            lambda: None, os.devnull, sock=object())
        with self.assertRaisesRegex(ValueError, 'path and sock can not be'):
            self.loop.run_until_complete(coro)

    def test_create_unix_datagram_connection_nopath_nosock(self):
        coro = self.loop.create_unix_datagram_connection(
            lambda: None, None)
        with self.assertRaisesRegex(ValueError,
                                    'path was not specified, and no sock'
