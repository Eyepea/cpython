...
class BaseSelectorEventLoop(base_events.BaseEventLoop):
    ...
    def _make_socket_datagram_transport(self, sock, protocol, address,
                                        waiter=None, extra=None):
        return _SelectorSocketDatagramTransport(self, sock, protocol, address,
                                                waiter, extra)
...
class _SelectorSocketDatagramTransport(_SelectorTransport):
    ...
    def _read_ready(self):
        ...
        try:
            ...
            self._protocol.datagram_received(data)
    ...
    def _send_ready(self):
        while self._buffer:
            ...
            try:
                ...
                self._buffer.appendleft(data)  # Try again later.
                break
            except OSError as exc:
                self._protocol.error_received(exc)
                return
    ...
    def wait_closed(self):
        if self._closing_waiter:
            yield from self._closing_waiter
        else:
            return
    ...
    def _call_connection_lost(self, exc):
        ...
        if self._closing_waiter:
            self._closing_waiter.set_result(None)
